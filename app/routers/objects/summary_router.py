...
from app.service.objects.summary_service import get_object_summary_service
...
def get_object_attributes_route(object_id: int, user_id: str = Depends(get_current_user)):
    return get_object_summary_service(object_id)

@router.patch("/{object_id}")
def update_attributes_route(object_id: int, updates: dict, user_id: str = Depends(get_current_user)):
    """
